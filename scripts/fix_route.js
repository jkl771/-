...
// 1. 修复情绪过滤（移除对特定音色的情绪限制）
const oldEmotion = `const edgeEmotion = (["yunxi","yunyang","yunjian","wanlung"].includes(edgeVoiceKey) && !["neutral","serious","warm"].includes(edgeEmotionRaw)) ? 'neutral' : edgeEmotionRaw`;
const newEmotion = `const edgeEmotion = edgeEmotionRaw`;
content = content.replace(oldEmotion, newEmotion);
console.log('Fixed emotion filter');
