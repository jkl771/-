...
import SourceSelector from '@/app/tts/components/SourceSelector';
import HistoryList from '@/app/tts/components/HistoryList';
import ProbePanel from '@/app/tts/components/ProbePanel';
...
export default function TTSPage() {
  ...
  // Ctrl+Enter / Cmd+Enter 快捷键合成
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter' && !synthesizing && text.trim()) {
        handleSynthesize();
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        <header>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight">🎤 声音工作台</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">声音克隆、语音合成、音色管理，支持多种 TTS 源</p>
        </header>

        {/* === 音源选择器（全宽） === */}
        <SourceSelector ttsMode={ttsMode} setTtsMode={setTtsMode} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {ttsMode === 'edge' && (
            <section className="lg:col-span-1 card space-y-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">免费预设音色，无需配置任何 Key，直接使用。</p>
                ...
                {edgeVoices.map((v) => (
                  <button key={v.key} onClick={() => setSelectedVoice(v.key)} className={'w-full p-3 rounded-xl border-2 text-left transition ' + (selectedVoice === v.key ? 'border-violet-400 bg-violet-50' : 'border-gray-200 bg-white hover:bg-gray-50')}>
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-sm text-gray-800 dark:text-gray-200">{v.label}</span>
                      <span className="text-xs text-gray-400 dark:text-gray-500">{v.style}</span>
                    </div>
                    <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">{v.desc}</div>
                  </button>
                ))}
              </div>
            </section>
          ...
          {ttsMode === 'cosy-clone' && (
            <section className="lg:col-span-1 card space-y-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">上传一段清晰人声，AI 自动克隆音色并可用于合成。</p>
              <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 rounded-xl p-3 text-xs text-blue-700 space-y-1">
              ...
              <label className="flex items-center justify-center gap-2 p-3 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl cursor-pointer hover:border-violet-300 hover:bg-violet-50 dark:bg-violet-900/30/40 transition text-xs text-gray-400 dark:text-gray-500">
              ...
              {clonedVoices.length > 0 && (
                <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-slate-700">
                  <div className="text-xs text-gray-500 dark:text-gray-400">已克隆音色</div>
                  {clonedVoices.map((v) => (
                    <div key={v.voiceId} className={'flex items-center justify-between p-2 rounded-lg text-xs transition ' + (selectedCloneId === v.voiceId ? 'bg-violet-50 border-2 border-violet-300' : 'bg-gray-50 border border-transparent')}>
                      {editingId === v.voiceId ? (
                        <div className="flex-1 flex gap-2">
                          ...
                          <button className="text-gray-400 dark:text-gray-500" onClick={() => setEditingId(null)}>取消</button>
                        </div>
                      ) : (
                        <>
                          <div className="flex-1 cursor-pointer" onClick={() => { setSelectedCloneId(v.voiceId); setTtsMode('cosy-clone'); }}>
                            <div className="font-medium text-gray-800 dark:text-gray-200">{v.name}</div>
                            <div className="text-gray-400 dark:text-gray-500 mt-0.5">{v.voiceId.slice(0, 24)}...</div>
                          </div>
          ...
          {ttsMode === 'elevenlabs' && (
            <section className="lg:col-span-1 card space-y-4">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">海外顶级 TTS，需配置 ElevenLabs Key。</p>
              {!elKeyConfigured && <div className="text-xs text-amber-600 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 rounded-xl p-3">请先在右侧配置 ElevenLabs API Key</div>}
              <div className="space-y-2">
                {elVoices.map((v) => (
                  <button key={v.key} onClick={() => setSelectedElVoice(v.key)} className={'w-full p-3 rounded-xl border-2 text-left transition ' + (selectedElVoice === v.key ? 'border-violet-400 bg-violet-50' : 'border-gray-200 bg-white hover:bg-gray-50')}>
                    <div className="font-medium text-sm text-gray-800 dark:text-gray-200">{v.label}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500">{v.desc}</div>
                  </button>
                ))}
              </div>
            </section>
          ...
          <section className="lg:col-span-2 card space-y-5">
            ...
            <div className="bg-gray-50 dark:bg-slate-900 rounded-xl p-3 flex items-center justify-between">
              <div>
                <span className="text-xs text-gray-400 dark:text-gray-500">当前音色</span>
            ...
            {/* 文案输入 */}
            <textarea className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300 min-h-[180px] resize-y" value={text} onChange={(e) => setText(e.target.value)} placeholder="在这里输入你想合成的文案..." />

            {/* 语音参数 */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">语速 <span className="text-gray-400 dark:text-gray-500">{speed.toFixed(2)}x</span></div>
                <input type="range" min="0.6" max="1.6" step="0.05" value={speed} onChange={(e) => setSpeed(parseFloat(e.target.value))} className="w-full" />
              </div>
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">音调 <span className="text-text-gray-400 dark:text-gray-500">{pitch}</span></div>
                <input type="range" min="-50" max="50" step="1" value={pitch} onChange={(e) => setPitch(parseInt(e.target.value))} className="w-full" />
              </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">句间停顿 <span className="text-gray-400 dark:text-gray-500">{pauseDuration}ms</span></div>
              ...
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">情绪</div>
                <select className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" value={emotion} onChange={(e) => setEmotion(e.target.value)}>
                  {EMOTIONS.map((em) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">背景音乐（可选）</div>
                <div className="flex gap-2">
                  <input className="flex-1 border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs" placeholder="输入关键词搜索 BGM" value={bgmQuery} onChange={(e) => setBgmQuery(e.target.value)} />
                  <button onClick={async () => { setBgmSearching(true); try { const r = await fetch(bgmQuery.trim()?'/api/bgm?source=audius&q='+encodeURIComponent(bgmQuery.trim()):'/api/bgm'); const d = await r.json(); if (d?.success) setBgmItems(d.data); } catch {} finally { setBgmSearching(false); } }} className="px-4 py-2 rounded-xl bg-violet-50 dark:bg-violet-900/300 text-white text-xs hover:bg-violet-600 transition whitespace-nowrap">{bgmSearching ? '...' : '搜索'}</button>
                  <label className="px-4 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/300 text-white text-xs hover:bg-emerald-600 transition whitespace-nowrap cursor-pointer">
                    {uploadingBgm ? '...' : '上传'}
                  ...
                  {filterByQuery(bgmItems, bgmQuery).slice(0, 10).map((b) => (
                    <div key={b.id} className={'flex items-center justify-between p-2 rounded-lg text-xs transition ' + (bgmFile === b.url ? 'bg-violet-50 border border-violet-200' : 'bg-gray-50 hover:bg-gray-100')}>
                      <button className="flex-1 text-left" onClick={() => setBgmFile(b.url)}>
                        ...
                        <div className="text-gray-400 dark:text-gray-500">{b.category}{b.artist ? ' · ' + b.artist : ''}</div>
                      </button>
                      <button onClick={(e) => {
                        e.stopPropagation();
                        if (previewBgm === b.url) {
              ...
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">BGM 音量 <span className="text-gray-400 dark:text-gray-500">{bgmVolume.toFixed(2)}</span></div>
            ...
            {/* 错误 */}
            {synthError && <div className="text-xs text-red-600 bg-red-50 dark:bg-red-900/30 border border-red-200 rounded-xl p-3">{synthError}</div>}

            {/* 降级提示 */}
            {synthResult?.autoSwitch && (
              <div className="text-xs text-amber-700 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 rounded-xl p-3">
                ⚠️ {synthResult.autoNote || '音源不可用，已自动切换到可用音源'}
              </div>
            )}
            {synthResult?.autoNote && !synthResult.autoSwitch && (
              <div className="text-xs text-blue-600 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 rounded-xl p-3">
            ...
            {/* 生成结果 */}
            {synthResult && (
              <div className="space-y-3 bg-gray-50 dark:bg-slate-900 rounded-xl p-4">
                <div className="flex items-center gap-3">
                  <Badge variant="success">✅ 生成成功</Badge>
                  <span className="text-xs text-gray-500 dark:text-gray-400">音源: {synthResult.providerUsed || ttsMode}</span>
                  {synthResult.duration ? <span className="text-xs text-gray-500 dark:text-gray-400">时长: {Math.round(synthResult.duration)}s</span> : null}
                </div>
            ...
            <ProbePanel
              ...
              onToggle={() => { if (showProbe) { setShowProbe(false); } else { setShowProbe(true); handleProbe(); } }}
            />

            {/* API Key 配置区 */}
            <div className="border-t border-gray-100 dark:border-slate-700 pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">DashScope Key（阿里云）</div>
                <Badge variant={apiKeyConfigured ? 'success' : 'default'}>{apiKeyConfigured ? '已配置' : '新配置'}</Badge>
                {apiKeyConfigured && <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 font-mono">{apiKeyMasked}</p>}
                {showApiKey ? (
                  <div className="space-y-2 mt-2">
                    <input className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-violet-300" type="password" placeholder="sk-..." value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} />
                    <div className="flex gap-2">
                      <button className="flex-1 py-2 rounded-xl text-white text-xs font-medium bg-violet-50 dark:bg-violet-900/300 hover:bg-violet-600 transition" onClick={handleSaveApiKey} disabled={apiKeyTesting}>{apiKeyTesting ? '...' : '保存'}</button>
                      <button className="py-2 px-3 rounded-xl text-xs text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:bg-slate-900 transition" onClick={() => setShowApiKey(false)}>取消</button>
                    </div>
              ...
              <div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">ElevenLabs Key</div>
                <Badge variant={elKeyConfigured ? 'success' : 'default'}>{elKeyConfigured ? '已配置' : '未配置'}</Badge>
                {elKeyConfigured && <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 font-mono">{elKeyMasked}</p>}
                {showUsage ? (
                  <div className="space-y-2 mt-2">
                    <input className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-violet-300" type="password" placeholder="EL Key..." value={elKeyInput} onChange={(e) => setElKeyInput(e.target.value)} />
                    <div className="flex gap-2">
                      <button className="flex-1 py-2 rounded-xl text-white text-xs font-medium bg-violet-50 dark:bg-violet-900/300 hover:bg-violet-600 transition" onClick={handleSaveElKey} disabled={elKeyTesting}>{elKeyTesting ? '...' : '保存'}</button>
                      <button className="py-2 px-3 rounded-xl text-xs text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:bg-slate-900 transition" onClick={() => setShowUsage(false)}>取消</button>
                    </div>
        ...
        <footer className="text-center text-xs text-gray-300 dark:text-gray-600 pt-8 pb-2">
