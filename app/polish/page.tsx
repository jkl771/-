...
function ScoreBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex items-center gap-2 text-xs py-0.5">
      <span className="w-14 text-gray-500 dark:text-gray-400 text-right shrink-0">{label}</span>
      ...
      <span className="w-7 text-gray-600 dark:text-gray-400 font-medium text-right shrink-0">{value}</span>
    </div>
  );
}
function ScorePanel({ score, title, compact }: { score: any; title: string; compact?: boolean }) {
  ...
  if (compact) {
    return (
      <div className="bg-gray-50 dark:bg-slate-900 rounded-xl p-3 space-y-0.5">
        <div className="flex justify-between items-center mb-1.5">
          <span className="text-xs font-medium text-gray-600 dark:text-gray-400">{title}</span>
          <span className={"text-base font-bold " + scoreColor(score.overall)}>{score.overall}</span>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 space-y-0.5">
      <div className="flex justify-between items-center mb-2">
        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{title}</span>
        <div className="flex items-center gap-2">
          <span className={"text-2xl font-bold " + scoreColor(score.overall)}>{score.overall}</span>
          <span className="text-xs text-gray-400 dark:text-gray-500">分</span>
        </div>
      </div>
      <ScoreBar label="可读性" value={score.readability} color="#3b82f6" />
      <ScoreBar label="信息量" value={score.infoDensity} color="#8b5cf6" />
      ...
      <ScoreBar label="风险" value={100 - score.forbiddenRisk} color="#22c55e" />
      {score.aiDetection !== undefined && <ScoreBar label="人味" value={100 - score.aiDetection} color="#a855f7" />}
      {score.method && <div className="text-xs text-gray-400 dark:text-gray-500 pt-1">{score.method === 'llm' ? '🤖 AI评分' : '📐 规则评分'}</div>}
      {score.details?.length > 0 && (
        <div className="mt-2 pt-2 border-t border-gray-100 dark:border-slate-700 space-y-0.5">
          {score.details.map((d: string, i: number) => <div key={i} className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{d}</div>)}
        </div>
      )}
    </div>
  );
...
function PolishPageInner() {
  ...
  if (!initialized) return <div className="text-center py-20 text-gray-400 dark:text-gray-500">加载中...</div>;
  return (
    <div className="space-y-5">
      {/* 标题 */}
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-bold text-gray-800 dark:text-gray-200">✨ 文案润色</h1>
        <a href="/materials-library" className="text-sm text-blue-500 hover:text-blue-700 transition-colors">📚 素材库</a>
      </div>
      {/* 输入区 */}
      <div className="card">
        <textarea className="w-full h-36 p-3 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-blue-400 focus:bg-white dark:bg-slate-800 resize-y transition-colors" placeholder="粘贴需要润色的文案..." value={text} onChange={e => setText(e.target.value)} />
        <div className="flex items-center gap-2 mt-3">
          <div className="flex rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden text-sm shrink-0">
          ...
          {tokenUsage && <span className="text-xs text-gray-400 dark:text-gray-500 ml-1 shrink-0">🪙 {tokenUsage.total_tokens}t</span>}
        </div>
        {/* LLM 配置（折叠） */}
        {/* AI 润色开关 */}
        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-slate-700">
          <div className="flex items-center gap-3 mb-2">
            ...
            {aiEnabled && <span className="text-xs text-gray-400 dark:text-gray-500">固定模型: GLM-4-Flash</span>}
            <span className="text-xs text-gray-300 dark:text-gray-600 mx-1">|</span>
            <button onClick={() => setShowLLM(!showLLM)} className="text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-400 transition-colors">
              📦 自定义配置{showLLM ? ' ▲' : ' ▼'}{llmConnected && <span className="ml-1.5 text-green-500">●</span>}
            </button>
          </div>
          {showLLM && (
            <div className="space-y-2 p-3 bg-gray-50 dark:bg-slate-900 rounded-xl">
              <div className="text-xs text-gray-400 dark:text-gray-500 mb-1">填写自己的 API 配置（优先级高于固定模型）</div>
      ...
      {error && <div className="card-compact border-red-200 bg-red-50 dark:bg-red-900/30"><p className="text-red-600 text-sm">❌ {error}</p></div>}
      {addMsg && <div className="card-compact border-blue-200 bg-blue-50 dark:bg-blue-900/30"><p className="text-blue-600 text-sm">{addMsg}</p></div>}
      {result && (
        <>
          {/* 合规报告 + 评分详情 合并 */}
          <div className="card-compact">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className={"text-lg font-bold " + ((editCompliance?.passed ?? result.complianceReport.passed) ? "text-emerald-600" : "text-red-600")}>
              ...
              <div className="flex items-center gap-1">
                {(editScore ?? result.polishedScore) && (
                  <span className={"text-xl font-bold " + scoreColor((editScore ?? result.polishedScore).overall)}>
                    {(editScore ?? result.polishedScore).overall}<span className="text-xs text-gray-400 dark:text-gray-500 ml-0.5">分</span>
                  </span>
                )}
                {(editScore ?? result.polishedScore)?.method && <span className="text-xs text-gray-400 dark:text-gray-500 ml-1">{(editScore ?? result.polishedScore).method === 'llm' ? '🤖' : '📐'}</span>}
              </div>
            </div>
            ...
            {((editCompliance?.issues ?? result.complianceReport.issues) || []).length > 0 && (
              <div className="mb-2">
                {(editCompliance?.issues ?? result.complianceReport.issues)?.map((issue: any, i: number) => (
                  <div key={i} className="text-xs text-red-500 mb-0.5 bg-red-50 dark:bg-red-900/30 rounded px-2 py-1">• <span className="font-medium">{issue.word}</span> → {issue.suggestion}</div>
                ))}
              </div>
            )}
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300">📝 润色结果</h2>
              <div className="flex items-center gap-2">
                ...
                {isEditing && <button onClick={handleSubmitScore} disabled={scoringEdit} className="text-xs px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 hover:bg-blue-100 transition-colors">{scoringEdit ? '⏳' : '📊 提交评分'}</button>}
              </div>
            </div>
            <textarea className="w-full h-32 p-3 rounded-xl border border-gray-200 dark:border-slate-700 bg-sky-50/50 text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-blue-400 resize-y" value={editText} onChange={e => handleEditChange(e.target.value)} />
          ...
          {/* 多版本 */}
          <div className="card-compact">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">🔄 多版本改写（{result.versions.length}）</h2>
                ...
                return (
                  <details key={i} className={`rounded-xl ${isBest ? 'bg-green-50 border border-green-200' : 'bg-gray-50'}`}>
                    <summary className="p-3 cursor-pointer text-sm flex justify-between items-center">
                      ...
                      {vs && <span className={"text-xs font-bold px-2 py-0.5 rounded-full " + scoreBg(vs.overall) + " " + scoreColor(vs.overall)}>{vs.overall}分</span>}
                    </summary>
                    <div className="px-3 pb-2 text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap leading-relaxed">{v}</div>
